...
class ColumnManager {
    ...
    hasLeftFrozenColumns() {
        return this._cache('hasLeftFrozenColumns', () => {
            return (this._fixed &&
                this.getVisibleColumns().some((column) => column.frozen === Column_1.FrozenDirection.LEFT || column.frozen === true));
        });
    }
    hasRightFrozenColumns() {
        return this._cache('hasRightFrozenColumns', () => {
            return (this._fixed && this.getVisibleColumns().some((column) => column.frozen === Column_1.FrozenDirection.RIGHT));
        });
    }
    getMainColumns() {
        return this._cache('mainColumns', () => {
            ...
            this.getLeftFrozenColumns().forEach((column) => {
                ...
                mainColumns.push(Object.assign({}, column, { [ColumnManager.PlaceholderKey]: true }));
            });
    ...
    getLeftFrozenColumns() {
        return this._cache('leftFrozenColumns', () => {
            ...
            return this.getVisibleColumns().filter((column) => column.frozen === Column_1.FrozenDirection.LEFT || column.frozen === true);
        });
    }
    getRightFrozenColumns() {
        return this._cache('rightFrozenColumns', () => {
            ...
            return this.getVisibleColumns().filter((column) => column.frozen === Column_1.FrozenDirection.RIGHT);
        });
    }
    getColumn(key) {
    ...
    recomputeColumnStyle(column) {
        ...
        const style = Object.assign({}, column.style, { flex: flexValue, msFlex: flexValue, WebkitFlex: flexValue, width: column.width, overflow: 'hidden' });
